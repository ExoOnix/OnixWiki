...
export default function User() {
    ...
    const [abilitiesState, setAbilitiesState] = useState<Record<number, boolean>>(
        () =>
            Object.fromEntries(
                props.role.abilities.map((ability) => [ability.id, ability.isAssigned])
            )
    );
    ...
    const handleToggle = (id: number) => {
        setAbilitiesState((prev) => {
            ...
            router.post(route('admin.roles.setAbility', { role: props.role.id, ability: id, allow: newState[id] }), { preserveState: true, preserveScroll: true });
            return newState;
        });
    };
